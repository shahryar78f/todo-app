...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ placeholder, language = 'en', label, name, className, labelClassName, ...props }, ref) => {
    ...
    const inputClasses = twMerge(
      'w-full px-4 py-2 bg-[#F4F5FA]  shadow-[inset_4px_4px_4px_0px_#ECEEF6] text-[#495CC5] placeholder-[#96ACD5] placeholder:font-normal placeholder:text-[10px]  rounded-[10px] focus:outline-none  focus:border-transparent transition-all',
      inputTextAlign,
      placeholderAlign,
      isPersian && 'dir-rtl',
      className,
    ...
    const labelClasses = twMerge(
      'block mb-2 text-[10px] pr-2 pb-1 font-semibold text-[#2F3973]',
      labelAlign,
      labelClassName,
    );
